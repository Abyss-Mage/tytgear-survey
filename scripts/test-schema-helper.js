...
// Price Sensitivity Range Schema
const PriceSensitivityRangeSchema = z
  .object({
...
const SurveySubmissionSchema = z
  .object({
    ...
    // Section 9: Giveaway & Discount Claim (Email only)
  ...
  .refine(
    (data) => {
      if (data.want_updates === "Yes") {
        if (!data.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email.trim())) return false;
        if (!data.contact_consent) return false;
      }
      return true;
    },
    {
      message: "Please enter a valid email address and consent to receive updates & enter the giveaway",
      path: ["email"],
    }
  );
